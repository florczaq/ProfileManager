...
int ProfilesManager::interactiveProfilesList(bool withAddOption)
{
  ...
  {
    profileName = profiles.at(i).getName();
    line = "+";
    countSpaces = 12 - profileName.length() / 2;

    for (int j = 0; j < countSpaces; j++)
      line += ' ';
    line += profileName;
    for (int j = 0; j < 24 - countSpaces - profileName.length(); j++)
      line += ' ';
    line += "+\n";
  ...
  while (keyPressed != ARROW::ENTER)
  {
    ...
    cout << "+      Choose profile    +\n";
    cout << "+------------------------+\n";
    interactiveMenu(namesList, option);
    cout << "+------------------------+\n";
    keyPressed = getch();
    move(keyPressed, option, namesList.size());
  }
  return option;
}
...
void ProfilesManager::editProfile(string name)
{
  ...
  while (int(keyPressed) != 13)
  {
    ...
    keyPressed = _getch();
    move(keyPressed, option, menuOptions.size());
  }
  system("cls");
  switch (option)
  {
...
void ProfilesManager::deletePathsMenu(int profileIndex)
{
  ...
  for (auto path : profiles.at(profileIndex).getPaths())
